...
import type { EnrichedChange, ChangeLocation } from '../types/summary.types';
...
/**
 * Tracking state while traversing document
 */
interface TraversalContext {
  currentSection: string | null;      // Current heading text
  currentParagraphText: string;       // Full text of current paragraph (including deleted)
...
function createEnrichedChange(
  ...
  if (trackMark.type === 'trackFormat') {
    ...
    return {
      ...
      surroundingText,
      formatDetails: {
        added: after.map((m: ProseMirrorNode) => m.type).filter((t: string) => !before.some((b: ProseMirrorNode) => b.type === t)),
        removed: before.map((m: ProseMirrorNode) => m.type).filter((t: string) => !after.some((a: ProseMirrorNode) => a.type === t)),
      },
      charCount: text.length,
    };
  }
